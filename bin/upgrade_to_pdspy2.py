...
if len(glob.glob("chain.npy")) > 0:
    ...
    version = "emcee3"
elif len(glob.glob("samples.npy")) > 0:
    version = "dynesty"
else:
    print("Did not detect any pdspy results... exiting.")
    sys.exit(0)
...
changed_files += config.visibilities["file"]
...
if version == "emcee":
    ...
    changed_files += ["samples.npy","sampler.p"]
...
# Get the keys of parameters and the index of particular ones that need to be
# updated.
...
def calculate_correct_masses(params):
    # Generate the model.
    ...
    # Update the disk density by calculating the unnormalized density, i.e. how
    # we used to do it, and integrating to get the normalization.

    r_high = numpy.logspace(numpy.log10(m.disk.rmin), \
            numpy.log10(m.disk.rmax), 1000)
    ...
    env_density_new = m.envelope.density(m.grid.r, m.grid.theta, m.grid.phi)

    m.envelope.cavrfact = 1.0
    env_density_old = m.envelope.density(m.grid.r, m.grid.theta, m.grid.phi)
...
if version == "dynesty":
    ...
    print("** Please note that because of the way that dynesty is run, it is impossible to update sampler.p. This means you will not be able to resume an in-progress fit, nor will you be able to re-run disk_model_nested.py, flared_model_nested, etc. within this model directory and get correct results. The resulting samples.npy has been updated so that you can still work with the results. Otherwise, you will need to re-run the fit. **")
...
# Also update the visibility data to take the complex conjugate.
