     ...
     &                   NUMANG, MAXANG, QEXT, QSCA, QBS, GQSC, 
     &                   M1, M2, S21, D21)
...
c     .. Scalar Arguments ..
      ...
      REAL(kind=8), intent(in)    ::  RCORE, RSHELL, WVNO
      REAL(kind=8), intent(out)   ::   GQSC, QBS, QEXT, QSCA
      COMPLEX(kind=8), intent(in) ::  RINDCO, RINDSH
c     ..
c     .. Array Arguments ..

      REAL, intent(in), dimension(NUMANG) ::    MU
      REAL, intent(out), dimension(MAXANG, 2)  ::    D21, M1,
     &          M2, S21
c     ..
c     .. Local Scalars ..
      ...
      REAL(kind=8) ::  AA, AIM, AM1IM, AM1RE, ARE, BB, BIM, BM1IM,
     ...
     &                  XCORE, XSHELL, Y1, Y4
      ...
      COMPLEX(kind=8) :: AC, ACOE, ACOEM1, BC, BCOE, BCOEM1, CI, CZERO,
     &                DH1, DH2, DH4, DUMMY, DUMSQ, K1, K2, K3, 
     &                P24H21, P24H24, RRFX, SBACK, WM1
c     ..
c     .. Local Arrays ..

      REAL(kind=8), dimension(MXANG, 3) :: PI
      REAL(kind=8), dimension(MXANG) :: SI2THT
      REAL(kind=8), dimension(5) :: T
      REAL(kind=8), dimension(4) :: TA
      REAL(kind=8), dimension(MXANG, 3) :: TAU

      COMPLEX(kind=8), dimension(LL) :: ACAP
      COMPLEX(kind=8), dimension(MXANG, 2) :: S1, S2
      COMPLEX(kind=8), dimension(8):: U
      COMPLEX(kind=8), dimension(3, LL):: W
      COMPLEX(kind=8), dimension(2):: WFN
      COMPLEX(kind=8), dimension(4):: Z
c     ..
