...
def uvinvert(data,imsize=256,pixel_size=0.25,weights="natural", \
    ...
    if spectral:
        ...
        startchannel = where(abs(velocity - start_velocity*kms) == \
    ...
    for i in range(nchannels)+startchannel:
        ...
        response = zeros((imsize,imsize))

        for j in baselines:
            baseline_data = averaged_data.get_baselines(j)
            if beam:
                weight = baseline_data.weights[baseline_data.real != \
                            0.].sum()
            else:
            ...
            if weights == "natural":
                weights = gridded_data.weights.reshape((imsize, \
                    imsize)).T
            ...
            nonzero = rarray != 0.
            responsivity = zeros(xarray.shape)
        ...
        im = transpose(roll(transpose(im),-1,axis=0))[:,::-1].reshape( \
                (imsize,imsize,1))
        response = transpose(roll(transpose(response),-1, \
                axis=0))[:,::-1].reshape((imsize,imsize,1))
    ...
    if spectral:
        return Image(image,velocity=\
            velocity[startchannel:startchannel+nchannels])
    else:
        return Image(image)
